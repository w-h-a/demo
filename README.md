...
2. The Business Logic Layer (Service): This is the core orchestrator. This layer makes features like _Create a User_ possible. It coordinates the flow by orchestrating external dependencies via interfaces, although it does not itself perform outbound calls and indeed it neither speaks the language of the inbound layer nor the outbound layer. This layer is usually performing core business logic on domain objects like `User` (`src/api/user`). 
3. The Outbound Layer (Client/Adapter): These consist primarily of the implementations (`PostgresUserRepository`) of the interfaces (`IUserRepository`) that the business logic layer uses to orchestrate external dependencies to get the job done.
...
1. Unit Tests (`npm test`)
    ...
    * Method: Mock only inter-system dependencies (`IUserRepository`), not intra-system collaborators (`User`).
